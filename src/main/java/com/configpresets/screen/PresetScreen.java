...
public class PresetScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        // Clip-less manual list: draw rows that fall within the panel.
        graphics.enableScissor(listX, listY, listX + listW, listY + listH);
        int y = listY - scroll;
        for (int i = 0; i < filtered.size(); i++) {
            int rowTop = y + i * ROW_H;
            int rowBottom = rowTop + ROW_H;
            if (rowBottom >= listY && rowTop <= listY + listH) {
                drawRow(graphics, filtered.get(i), i, rowTop, mouseX, mouseY);
            }
        }
    ...
    private String trim(String s, int maxWidth) {
        ...
        for (char c : s.toCharArray()) {
            if (this.font.width(sb.toString() + c + "\u2026") > maxWidth) {
                break;
            }
            sb.append(c);
        }
